...
//  }
  ...
  MapsLib.prototype.sliderInterval = function(interval) {
    ...
    MapsLib.prototype.doSearch = function (s, e) {
        var self = this;
        self.clearSearch();
        ...
        //-----custom filters-----
        console.log('doSearch>>> start date is : ' + s);

        // if startdate and enddate are not exposed, let startdate be one year ago and enddate today
        var startdate = $('input[name="daterange"]').data('startdate')
							? $('input[name="daterange"]').data('startdate')
                            : moment().subtract(365, 'days').calendar();;
		var enddate = $('input[name="daterange"]').data('enddate')
							? $('input[name="daterange"]').data('enddate')
